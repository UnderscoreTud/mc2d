...
public final class ShelfData extends BlockData implements HasFacing, HasPowered, HasWaterlogged {
    ...
    private static final int SIDE_CHAIN_RADIX = 4;

    private static final int WATERLOGGED_RADIX = 2;

    static final int MAX_OFFSET = FACING_RADIX * POWERED_RADIX * SIDE_CHAIN_RADIX * WATERLOGGED_RADIX - 1;
    ...
    private SideChain sideChain = SideChain.UNCONNECTED;

    private boolean waterlogged = false;

    @ApiStatus.Internal
    public ShelfData(Block<?> blockType, int startingID) {
    ...
    public void sideChain(SideChain sideChain) {
        Preconditions.checkNotNull(sideChain, "sideChain");
        this.sideChain = sideChain;
    }

    private int sideChainToIndex() {
        return this.sideChain.ordinal();
    }

    private void sideChainFromIndex(int index) {
        this.sideChain(SideChain.values()[index]);
    }

    private void sideChainFromString(String value) {
        this.sideChain(SideChain.parse(value));
    }
    ...
    int offsetID() {
        ...
        id = id * SIDE_CHAIN_RADIX + sideChainToIndex();
    ...
    void loadFromOffset(int id) {
        ...
        sideChainFromIndex(id % SIDE_CHAIN_RADIX); id /= SIDE_CHAIN_RADIX;
    ...
    public void load(Map<String, String> properties) {
        ...
        if (properties.containsKey("side_chain")) {
            sideChainFromString(properties.get("side_chain"));
        }
    ...
    public String toString() {
        return blockType().key() + "[facing=" + String.valueOf(facing).toLowerCase(Locale.ENGLISH) + ";powered=" + String.valueOf(powered).toLowerCase(Locale.ENGLISH) + ";side_chain=" + String.valueOf(sideChain).toLowerCase(Locale.ENGLISH) + ";waterlogged=" + String.valueOf(waterlogged).toLowerCase(Locale.ENGLISH) + "]";
    }
    ...
    public enum SideChain {
        ...
        public static SideChain parse(String value) {
            return switch (value.toLowerCase(Locale.ENGLISH)) {
                case "unconnected" -> SideChain.UNCONNECTED;
                case "right" -> SideChain.RIGHT;
                case "center" -> SideChain.CENTER;
                case "left" -> SideChain.LEFT;
