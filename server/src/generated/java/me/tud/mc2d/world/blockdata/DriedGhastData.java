...
public final class DriedGhastData extends BlockData implements HasFacing, HasWaterlogged {
    ...
    private static final int HYDRATION_RADIX = 4;

    private static final int WATERLOGGED_RADIX = 2;

    static final int MAX_OFFSET = FACING_RADIX * HYDRATION_RADIX * WATERLOGGED_RADIX - 1;

    private @BlockFace.Values({BlockFace.NORTH, BlockFace.SOUTH, BlockFace.WEST, BlockFace.EAST}) BlockFace facing = BlockFace.NORTH;

    private @Range(from = 0, to = 3) int hydration = 0;

    private boolean waterlogged = false;

    @ApiStatus.Internal
    public DriedGhastData(Block<?> blockType, int startingID) {
    ...
    public @Range(from = 0, to = 3) int hydration() {
        return this.hydration;
    }

    public void hydration(@Range(from = 0, to = 3) int hydration) {
        Preconditions.checkArgument(0 <= hydration && hydration <= 3, "Value must be in [0, 3], got " + hydration);
        this.hydration = hydration;
    }

    private int hydrationToIndex() {
        return this.hydration;
    }

    private void hydrationFromIndex(int index) {
        this.hydration(index);
    }

    private void hydrationFromString(String value) {
        this.hydration(Integer.parseInt(value));
    }
    ...
    int offsetID() {
        ...
        id = id * HYDRATION_RADIX + hydrationToIndex();
    ...
    void loadFromOffset(int id) {
        ...
        hydrationFromIndex(id % HYDRATION_RADIX); id /= HYDRATION_RADIX;
    ...
    public void load(Map<String, String> properties) {
        ...
        if (properties.containsKey("hydration")) {
            hydrationFromString(properties.get("hydration"));
        }
    ...
    public String toString() {
        return blockType().key() + "[facing=" + String.valueOf(facing).toLowerCase(Locale.ENGLISH) + ";hydration=" + String.valueOf(hydration).toLowerCase(Locale.ENGLISH) + ";waterlogged=" + String.valueOf(waterlogged).toLowerCase(Locale.ENGLISH) + "]";
    }
}
