...
public final class EndPortalFrameData extends BlockData implements HasFacing {
    private static final int EYE_RADIX = 2;
    ...
    static final int MAX_OFFSET = EYE_RADIX * FACING_RADIX - 1;
    ...
    @ApiStatus.Internal
    public EndPortalFrameData(Block<?> blockType, int startingID) {
    ...
    public void eye(boolean eye) {
        this.eye = eye;
    }

    private int eyeToIndex() {
        return this.eye ? 0 : 1;
    }

    private void eyeFromIndex(int index) {
        this.eye(index == 0);
    }

    private void eyeFromString(String value) {
        this.eye(Boolean.parseBoolean(value));
    }
    ...
    void loadFromOffset(int id) {
        ...
        eyeFromIndex(id % EYE_RADIX); id /= EYE_RADIX;
    }

    @Override
    public void load(Map<String, String> properties) {
        if (properties.containsKey("eye")) {
            eyeFromString(properties.get("eye"));
        }
    ...
    public String toString() {
        return blockType().key() + "[eye=" + String.valueOf(eye).toLowerCase(Locale.ENGLISH) + ";facing=" + String.valueOf(facing).toLowerCase(Locale.ENGLISH) + "]";
    }
}
