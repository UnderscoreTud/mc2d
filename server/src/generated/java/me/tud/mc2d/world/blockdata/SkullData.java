package me.tud.mc2d.world.blockdata;

import com.google.common.base.Preconditions;
import java.util.Locale;
import java.util.Map;
import javax.annotation.processing.Generated;
import me.tud.mc2d.world.blockdata.properties.HasPowered;
import me.tud.mc2d.world.blockdata.properties.HasRotation;
import org.jetbrains.annotations.ApiStatus;
import org.jetbrains.annotations.Range;

/**
 * This class contains autogenerated code. Please do <b>NOT</b> edit manually.
 */
@Generated("me.tud.mc2d.generators.blocks.BlocksGenerator")
public final class SkullData extends BlockData implements HasPowered, HasRotation {
    private static final int POWERED_RADIX = 2;

    private static final int ROTATION_RADIX = 16;

    static final int MAX_OFFSET = POWERED_RADIX * ROTATION_RADIX - 1;

    private boolean powered = false;

    private @Range(from = 0, to = 15) int rotation = 0;

    @ApiStatus.Internal
    public SkullData(int startingID) {
        super(startingID);
    }

    @Override
    public boolean powered() {
        return this.powered;
    }

    @Override
    public void powered(boolean powered) {
        this.powered = powered;
    }

    private int poweredToIndex() {
        return this.powered ? 0 : 1;
    }

    private void poweredFromIndex(int index) {
        this.powered(index == 0);
    }

    private void poweredFromString(String value) {
        this.powered(Boolean.parseBoolean(value));
    }

    @Override
    public @Range(from = 0, to = 15) int rotation() {
        return this.rotation;
    }

    @Override
    public void rotation(@Range(from = 0, to = 15) int rotation) {
        Preconditions.checkArgument(0 <= rotation && rotation <= 15, "Value must be in [0, 15], got " + rotation);
        this.rotation = rotation;
    }

    private int rotationToIndex() {
        return this.rotation;
    }

    private void rotationFromIndex(int index) {
        this.rotation(index);
    }

    private void rotationFromString(String value) {
        this.rotation(Integer.parseInt(value));
    }

    @Override
    public int maxOffset() {
        return MAX_OFFSET;
    }

    @Override
    int offsetID() {
        int id = 0;
        id = id * POWERED_RADIX + poweredToIndex();
        id = id * ROTATION_RADIX + rotationToIndex();
        return id;
    }

    @Override
    void loadFromOffset(int id) {
        rotationFromIndex(id % ROTATION_RADIX); id /= ROTATION_RADIX;
        poweredFromIndex(id % POWERED_RADIX); id /= POWERED_RADIX;
    }

    @Override
    public void load(Map<String, String> properties) {
        if (properties.containsKey("powered")) {
            poweredFromString(properties.get("powered"));
        }
        if (properties.containsKey("rotation")) {
            rotationFromString(properties.get("rotation"));
        }
    }

    @Override
    public String toString() {
        return "SkullData[powered=" + String.valueOf(powered).toLowerCase(Locale.ENGLISH) + ";rotation=" + String.valueOf(rotation).toLowerCase(Locale.ENGLISH) + "]";
    }
}
