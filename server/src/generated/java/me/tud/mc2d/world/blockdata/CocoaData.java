...
public final class CocoaData extends BlockData implements HasAge, HasFacing {
    private static final int AGE_RADIX = 3;
    ...
    static final int MAX_OFFSET = AGE_RADIX * FACING_RADIX - 1;

    private @Range(from = 0, to = 2) int age = 0;

    private @BlockFace.Values({BlockFace.NORTH, BlockFace.SOUTH, BlockFace.WEST, BlockFace.EAST}) BlockFace facing = BlockFace.NORTH;

    @ApiStatus.Internal
    public CocoaData(Block<?> blockType, int startingID) {
        super(blockType, startingID);
    }

    @Override
    public @Range(from = 0, to = 2) int age() {
        return this.age;
    }

    @Override
    public void age(@Range(from = 0, to = 2) int age) {
        Preconditions.checkArgument(0 <= age && age <= 2, "Value must be in [0, 2], got " + age);
        this.age = age;
    }
    ...
    public String toString() {
        return blockType().key() + "[age=" + String.valueOf(age).toLowerCase(Locale.ENGLISH) + ";facing=" + String.valueOf(facing).toLowerCase(Locale.ENGLISH) + "]";
    }
}
