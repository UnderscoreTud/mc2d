...
public class PacketsGenerator extends JsonFileGenerator {
    ...
    public GeneratedType[] generate(JsonReader reader) throws IOException {
        ...
        reader.endObject();
        return new GeneratedType[]{new GeneratedType(getClass(), CLASS_NAME.packageName(), packetIDsType.build())};
    }
    ...
    private TypeSpec generateDirectionClass(String state, JsonReader reader) throws IOException {
        ...
        for (Map.Entry<String, Integer> packet : packets) {
            String fieldName = StringUtils.cleanNamespacedKey(packet.getKey()).toUpperCase(Locale.ENGLISH);
